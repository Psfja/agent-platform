...
### 第九轮修复（2026-08-22）：消灭死控件，删除/温度等操作全部真实可用

用户反馈（Temperature 滑块不动、推荐模型不能改、用户/智能体删不掉）后全面排查：

- **Temperature 成为真实字段**：AgentType 与 AgentBuild 新增 temperature 列（迁移 0006），
  创建/编辑智能体与发起构建均可设置（滑块实时显示），并真实作用于对话（chat/流式）、
  DeepAgents 工具模式、子智能体委派与分阶段代码生成调用；修复阶段保持低温度稳定输出。
- **推荐模型可任意输入**：模型控件改为输入框 + datalist 推荐项，不再限制于下拉选项。
- **智能体删除真正可用**：AgentType 响应新增 usage（引用节点数/模板名/进行中任务）；
  无引用的智能体一键删除；被引用的删除按钮禁用并显示引用来源，后端 409 带模板名详情。
- **用户删除真正可用**：删除对话框显示名下项目数 +「移交给我」勾选；
  后端支持 reassign=true 移交后删除（审计记录），不勾选则 409 并列出项目。
- **构建页死控件修复**：自动修复次数 → 真实下拉（1–5 次）；新增 Temperature 滑块随构建请求生效。
- **其余死控件清扫**：Agent 运行时上下文「Agent 类型」→ 可选下拉；代码仓库分支选择 → 构建切换下拉；
  迭代历史「全部/功能/修复」筛选可用；产物中心类型导航可用；任务树刷新按钮真实刷新；
