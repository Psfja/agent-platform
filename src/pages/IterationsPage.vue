...
import { iterations as fallbackIterations } from '../data/mock'
...
const route=useRoute();const router=useRouter();const app=useAppStore();const showChange=ref(false);const expanded=ref('i-130');const filter=ref('全部版本');const iterationItems=ref<Iteration[]>([...fallbackIterations])
async function load(){try{iterationItems.value=await api.iterations(String(route.params.id));expanded.value=iterationItems.value[0]?.id||''}catch{app.toast('后端连接提示','迭代历史继续显示本地演示数据。')}}
function compare(version:string){if(version==='v1.3.0'){app.toast('版本尚未发布','执行中的版本还没有生成正式快照。');return}router.push(`/projects/${route.params.id}/compare?from=v1.1.1&to=${version}`)}
onMounted(load)
</script>
...
<template>
  <div class="content-width iterations-page">
    ...
    <section class="version-overview">
      <div class="current-version"><span><Tag :size="22"/></span><div><small>当前生产版本</small><strong>v1.2.0</strong><p><i></i>运行稳定 · 已上线 4 天</p></div></div>
      <div class="version-stat"><small>累计迭代</small><strong>{{iterationItems.length}} <em>次</em></strong></div><div class="version-stat"><small>累计变更</small><strong>{{iterationItems.reduce((n,i)=>n+i.files,0)}} <em>文件</em></strong></div><div class="version-stat"><small>回归通过率</small><strong>98.4<em>%</em></strong></div>
      ...
      <article v-for="(item,index) in iterationItems" :key="item.id" class="iteration-item" :class="[`iteration-${item.status}`,{expanded:expanded===item.id}]">
        ...
        <div class="iteration-card">
          ...
          <div v-if="expanded===item.id" class="iteration-detail">
            ...
            <div class="detail-actions-row"><button class="text-button" @click="router.push(`/projects/${route.params.id}/iterations/${item.id}`)">查看完整详情<ArrowRight :size="15"/></button><button class="text-button" @click="compare(item.version)"><GitCompareArrows :size="15"/>与上一版本对比</button><button v-if="item.status==='completed'" class="text-button danger-text" @click="app.toast('回滚提示','请在版本对比页完成风险确认后回滚。')"><RotateCcw :size="15"/>回滚到此版本</button></div>
          </div>
        </div>
      </article>
    </section>
