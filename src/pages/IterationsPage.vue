...
const route=useRoute();const router=useRouter();const app=useAppStore();const showChange=ref(false);const expanded=ref('');const typeFilter=ref<'all'|'feature'|'fix'>('all');const iterationItems=ref<Iteration[]>([])
const visibleIterations=computed(()=>typeFilter.value==='all'?iterationItems.value:iterationItems.value.filter(item=>item.type===typeFilter.value))
...
<template>
  <div class="content-width iterations-page">
    ...
    <div class="iteration-toolbar"><div><button :class="{active:typeFilter==='all'}" @click="typeFilter='all'">全部迭代 <span>{{iterationItems.length}}</span></button><button :class="{active:typeFilter==='feature'}" @click="typeFilter='feature'">功能 <span>{{iterationItems.filter(i=>i.type==='feature').length}}</span></button><button :class="{active:typeFilter==='fix'}" @click="typeFilter='fix'">修复 <span>{{iterationItems.filter(i=>i.type==='fix').length}}</span></button></div></div>
    <section class="timeline">
      <article v-for="(item,index) in visibleIterations" :key="item.id" class="iteration-item" :class="[`iteration-${item.status}`,{expanded:expanded===item.id}]">
