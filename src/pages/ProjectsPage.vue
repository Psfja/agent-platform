...
<template>
  <div class="content-width projects-page">
    ...
    <section class="workspace-summary">
      <div class="summary-copy"><span class="summary-icon"><Sparkles :size="21"/></span><div><strong>上午好，林嘉</strong><p>2 个项目正在执行，员工假勤平台的回归测试已完成 68%。</p></div></div>
      <div class="summary-stats"><div><span class="dot indigo"></span><strong>{{ stats.active }}</strong><small>进行中</small></div><i></i><div><span class="dot green"></span><strong>{{ stats.done }}</strong><small>已交付</small></div><i></i><div><span class="dot amber"></span><strong>1</strong><small>待处理</small></div></div>
    </section>
